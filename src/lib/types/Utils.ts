...
export type GetTyped<T extends TypedT<unknown>> = T extends TypedT<infer U> ? U : never;
export function T<TCustom = string>(k: string): TypedT<TCustom> {
  return k as TypedT<TCustom>;
}

export type TypedFT<TArgs, TReturn = string> = string & { __type__: TArgs; __return__: TReturn };
